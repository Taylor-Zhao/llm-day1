...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Day25 task orchestration")
    parser.add_argument(
        "--question",
        default="请完成接口联调：先识别可用 endpoint，再请求 order_id=1001，最后总结联调结果",
        help="编排目标问题",
    )
    parser.add_argument(
        "--planner-prompt",
        default="prompts/day25_task_orchestrator_cn.txt",
        help="任务规划提示词路径",
    )
    ...
    parser.add_argument(
        "--report",
        default="experiments/day25_task_orchestration.md",
        help="Markdown 报告",
    )
    parser.add_argument(
        "--jsonl",
        default="logs/day25_task_orchestration.jsonl",
        help="编排步骤 JSONL",
    )
...
def execute_step(gateway: LocalToolGateway, step: dict[str, Any]) -> dict[str, Any]:
    ...
    if tool_name == "http_get":
        if "url" not in args:
            ...
            args = {
                "url": args.get("url"),
                "params": {k: v for k, v in args.items() if k != "url"},
            }
    ...
    *,
    ...
    lines.extend(
        [
            ...
            "- 若多次超时，可提升 --timeout-seconds（例如 25）。",
...
def write_report(
    ...
    *,
    ...
    for idx, step in enumerate(plan, start=1):
        ...
        lines.append(
            f"| {idx} | {step.get('tool')} | {int(ok)} | {(step.get('why') or '')[:120]} |"
        )
...
def main() -> None:
    ...
    try:
        summary_text, summary_usage = summarize_results(
            client=client,
            api_key=api_key,
            base_url=base_url,
            model=model_name,
