...
输出：
- Markdown 报告：experiments/day19_cache_dedup_comparison.md
- CSV 汇总：experiments/day19_cache_dedup_comparison.csv
- JSONL 明细：logs/day19_cache_dedup_comparison.jsonl
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Day19 cache + dedup comparison")
    ...
    parser.add_argument("--report", default="experiments/day19_cache_dedup_comparison.md", help="Markdown 报告")
    parser.add_argument("--csv", default="experiments/day19_cache_dedup_comparison.csv", help="CSV 汇总")
    parser.add_argument("--jsonl", default="logs/day19_cache_dedup_comparison.jsonl", help="JSONL 明细")
    ...
    *,
    ...
    if enable_cache:
        ...
        if probe is not None:
            probe.lookup(hit)
        if hit:
            return copy.deepcopy(caches.vector_candidates[cache_key])
...
def compute_mode_summary(rows: list[dict]) -> dict:
    ...
    elapsed_total = sum(float(r.get("elapsed_ms") or 0) for r in rows)
    ...
    return {
        ...
        "avg_elapsed_ms": elapsed_total / total,
        ...
        "dedup_total_removed": dedup_removed_total,
    }
...
def write_report(path: Path, args: argparse.Namespace, baseline_summary: dict, cache_dedup_summary: dict) -> None:
    ...
    delta_hit = cache_dedup_summary["retrieval_hit_rate"] - baseline_summary["retrieval_hit_rate"]
    delta_cite = cache_dedup_summary["citation_correct_rate"] - baseline_summary["citation_correct_rate"]
    delta_ins = cache_dedup_summary["insufficient_correct_rate"] - baseline_summary["insufficient_correct_rate"]
    delta_fmt = cache_dedup_summary["citation_format_valid_rate"] - baseline_summary["citation_format_valid_rate"]
    delta_tok = cache_dedup_summary["avg_total_tokens"] - baseline_summary["avg_total_tokens"]
    delta_latency = cache_dedup_summary["avg_elapsed_ms"] - baseline_summary["avg_elapsed_ms"]
    ...
    lines = [
        ...
        "|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|",
        (
            f"| baseline | {baseline_summary['query_count']} | {baseline_summary['retrieval_hit_rate']:.3f} | "
            f"{baseline_summary['citation_correct_rate']:.3f} | {baseline_summary['insufficient_correct_rate']:.3f} | "
            f"{baseline_summary['citation_format_valid_rate']:.3f} | {baseline_summary['avg_total_tokens']:.1f} | "
            f"{baseline_summary['avg_elapsed_ms']:.1f} | {baseline_summary['cache_hit_rate']:.3f} | "
            f"{baseline_summary['avg_dedup_removed']:.2f} |"
        ),
        (
            f"| cache_dedup | {cache_dedup_summary['query_count']} | {cache_dedup_summary['retrieval_hit_rate']:.3f} | "
            f"{cache_dedup_summary['citation_correct_rate']:.3f} | {cache_dedup_summary['insufficient_correct_rate']:.3f} | "
            f"{cache_dedup_summary['citation_format_valid_rate']:.3f} | {cache_dedup_summary['avg_total_tokens']:.1f} | "
            f"{cache_dedup_summary['avg_elapsed_ms']:.1f} | {cache_dedup_summary['cache_hit_rate']:.3f} | "
        ...
        f"- avg_elapsed_ms: {delta_latency:+.1f}",
        f"- cache_hit_rate(cache_dedup): {cache_dedup_summary['cache_hit_rate']:.3f}",
        f"- dedup_total_removed(cache_dedup): {cache_dedup_summary['dedup_total_removed']}",
        "",
        "## 技术图解（Mermaid）",
        "",
...
def evaluate_case(
    *,
    ...
    vector_candidates = retrieve_vector_hits(
...
def main() -> None:
    ...
    print("Done. Day19 cache + dedup comparison generated.")
